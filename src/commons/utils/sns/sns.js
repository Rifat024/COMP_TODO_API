...
export const jOtp_lock_unlock_history = async (
 ...
 // recordLocal = JSON.parse(replaceChnlValRecord);
  ...
  console.log("publishTextPromise SNS_JOTP_LOCK_UNLOCK_HISTORY  params ===>", params);

  let publishTextPromise = await sns.publish(params).promise();
  console.log("publishTextPromise SNS_JOTP_LOCK_UNLOCK_HISTORY ===>", publishTextPromise);
...
export const jOtp_store_journey = async (recordLocal) => {
...
export const jOtp_system_trigger = async (recordLocal) => {
 

  var params = {
    Message: JSON.stringify(recordLocal),
    TopicArn: AppConfig.SNS_JOTP_SYSTEM_TRIGGER,
  };
  console.log("publishTextPromise SNS_JOTP_SYSTEM_TRIGGER  params ===>", params);

  let publishTextPromise = await sns.publish(params).promise();
  console.log("publishTextPromise SNS_JOTP_SYSTEM_TRIGGER ===>", publishTextPromise);
